 ...
 * Description:       Improve the way you manage deliveries, drivers, and order assignments. With the premium version, unlock even more features including SMS and email notifications, route planning, navigation, and more!
 * Version:           3.0.0
 * Author:            powerfulwp
...
if ( !function_exists( 'lddfw_activate' ) ) {
    ...
    function lddfw_premium_feature_notice_content(  $html  ) {
		...
		<div class="lddfw_premium-feature-content"><div class="lddfw_title premium_feature_title">
		 ...
		 <p>' . esc_html( __( 'Unlock this feature with Premium.', 'lddfw' ) ) . '</p>
		</div>
		 <p class="lddfw_content-subtitle">' . esc_html( __( 'With the premium version, you will be able to:', 'lddfw' ) ) . '</p>
		' . $html . '</div>';
    }
    ...
    function lddfw_premium_feature_notice(  $button, $html, $class  ) {
        $trigger_label = ( $button ? ' <span>' . esc_html( $button ) . '</span>' : '' );
        return '<div class="lddfw_premium-feature ' . esc_attr( $class ) . '">
			<button type="button" class="lddfw_premium-trigger-btn">' . lddfw_premium_feature( '' ) . $trigger_label . '</button>
			<div class="lddfw_lightbox lddfw_premium-modal" style="display:none" role="dialog" aria-modal="true">
				<div class="lddfw_lightbox_wrap">
					<div class="container">
						<button type="button" class="lddfw_lightbox_close lddfw_premium-modal__close" aria-label="' . esc_attr__( 'Close', 'lddfw' ) . '">
						...
						' . lddfw_premium_feature_notice_content( $html ) . '
						<button type="button" class="lddfw_lightbox_close lddfw_premium-godit-btn">' . esc_html__( 'Got it', 'lddfw' ) . '</button>
					</div>
				</div>
			</div>
		</div>';
    ...
    function lddfw_admin_notices() {
        if ( !class_exists( 'WooCommerce' ) ) {
            echo '<div class="notice notice-error is-dismissible">
				<p>' . esc_html( __( 'Local Delivery Drivers for WooCommerce is a WooCommerce add-on. Please activate WooCommerce on your site before continuing.', 'lddfw' ) ) . '</p>
				</div>';
        }
    }
